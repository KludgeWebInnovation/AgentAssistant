...
def generate_sequence(
    ...
    contact.last_activity_at = utcnow()
    session.add(AuditEvent(contact_id=contact.id, event_type="sequence_generated", detail=f"Sequence created from template {template.name}."))
    session.flush()
    return sequence
...
def approve_step(session: Session, contact: Contact, step: SequenceStep) -> None:
    ...
    session.add(AuditEvent(contact_id=contact.id, event_type="step_approved", detail=f"Approved step {step.step_order}."))
...
def record_manual_send(session: Session, contact: Contact, step: SequenceStep, audit_note: str = "") -> None:
    ...
    session.add(AuditEvent(contact_id=contact.id, event_type="step_sent", detail=f"Recorded manual send for step {step.step_order}."))


def pause_contact(session: Session, contact: Contact, reason: str, terminal_state: LeadState) -> None:
    for sequence in session.exec(select(Sequence).where(Sequence.contact_id == contact.id, Sequence.status.in_(["active", "paused"]))).all():
